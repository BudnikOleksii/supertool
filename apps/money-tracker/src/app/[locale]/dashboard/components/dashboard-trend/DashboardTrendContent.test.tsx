...
import { checkHasChartColors, resolveChartColors } from './DashboardTrendContent';

const LIGHT_PALETTE: Record<string, string> = {
  '--on-success-container': '#1b6e2d',
  '--error': '#b3261e',
...
describe('resolveChartColors', () => {
  ...
  it('reflects the active theme palette on each read so a theme change yields new colors', () => {
    ...
    expect(light.income).toBe('#1b6e2d');
    expect(dark.income).toBe('#7ee896');
...
describe('checkHasChartColors', () => {
  ...
  it('returns true once the income token resolves', () => {
