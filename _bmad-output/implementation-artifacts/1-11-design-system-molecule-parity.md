...
Status: in-progress (9/10 molecules complete with QA + gates; Task 8 `field` blocked on Story 1.10's `Separator` atom — see Dev Agent Record)
...
- [ ] **Task 8 — `field` (LAST — needs the 1.10 `Separator`) (AC: #1, #5) — DRAFTED, NOT YET VERIFIED. Blocked on Story 1.10 (`Separator` atom not yet merged; 1.10 status = `backlog`). The component is written and validated against a reference-accurate temporary `Separator` stub (type-check + 4 tests + story type-check all passed, stub then removed). It remains unchecked because: (a) it has NOT been validated against the REAL 1.10 `Separator`, and (b) its visual QA (AC-5) is deferred — the separator line cannot be screenshot-verified without the real atom. While `field` is present, `packages/ui` will NOT type-check/build until 1.10 merges (the `../../atoms/separator/Separator` import is unresolved).**
  - [x] PRECONDITION: confirm `packages/ui/src/components/atoms/separator/Separator.tsx` exists (delivered by 1.10, running in parallel). If it has not landed yet, do Tasks 9–10 for the other nine molecules first and return to `field` once `Separator` is merged (see "Coordination with Story 1.10"). → CHECKED: `atoms/separator/Separator.tsx` does NOT exist; current atoms = `button`, `input`, `label`, `select`, `typography` only. Tasks 9–10 executed for the nine molecules; `field` held until 1.10 lands.
  - [ ] `molecules/field/Field.{tsx,module.scss,test.tsx}` — composes `Label` + `Separator` (1.10). DRAFTED (validated vs a temp stub). When 1.10 lands: confirm the real `Separator` export name/path matches `import { Separator } from '../../atoms/separator/Separator'` and that it accepts `className` for `FieldSeparator`'s line; adjust if 1.10's surface differs. Then run the gates + visual QA.
  - [ ] CSF3 story showing a labeled field with description + error state and a `FieldSet`/`FieldGroup` grouping. DRAFTED as `apps/storybook/src/stories/Field.stories.tsx` (Default, WithError, GroupedSignUp). Visual QA pending real `Separator`.
...
- Shipped 9 of the 10 molecules complete (component + token-only `.module.scss` + co-located `*.test.tsx` + CSF3 story under `apps/storybook/src/stories/`): `accordion`, `alert-dialog`, `breadcrumb`, `card`, `combobox` (+ 4 hooks), `dropdown-menu`, `error-state`, `pagination`, `toaster`.
- **Task 8 (`field`) is BLOCKED and deferred.** Its `FieldSeparator` composes the `Separator` atom delivered by **Story 1.10**, which is `backlog` and absent from `packages/ui/src/components/atoms/` (verified: atoms = `button`, `input`, `label`, `select`, `typography` only). Per the story's Coordination section, the other nine molecules were completed with full visual QA + gates; `field` + its QA land once 1.10's `Separator` merges. The story is therefore NOT yet `done`/`review` — it remains `in-progress` pending 1.10.
- Tests: 44 passing in `@supertool/ui` (16 files), including Radix-in-jsdom open-state coverage (accordion expand, dropdown/alert-dialog/combobox open via `fireEvent`, combobox keyboard nav + type-to-filter). No repo regressions (shell 10, api 19 still green).
...
**Drafted but UNVERIFIED (Task 8, pending Story 1.10's `Separator`):** `molecules/field/Field.tsx`, `molecules/field/Field.module.scss`, `molecules/field/Field.test.tsx`, `apps/storybook/src/stories/Field.stories.tsx`. Validated against a temporary reference-accurate `Separator` stub (type-check + 4 tests + story type-check passed; stub removed). These import `../../atoms/separator/Separator`, which does not exist yet — so `packages/ui` will NOT type-check/build until 1.10 merges. No `field` visual QA captured (needs the real `Separator`).
