...
| `@radix-ui/react-slot` | `1.2.5` | sanctioned in code review (Oleksii) — required for `BreadcrumbLink`'s `asChild`/Slot escape hatch (reference parity, type-safe); see Review Findings |
...
| 2026-06-13 | code-review (bmad-code-review, 3-layer: Blind Hunter / Edge Case Hunter / Acceptance Auditor). Gates independently re-run green (lint, fmt:check, stylelint, type-check --force, test --force, i18n:parity, build w/ sass, generate:client byte-identical). No blocking findings; Acceptance Auditor declared acceptance-clean on all 5 ACs. Findings recorded below: 1 patch, 2 decision-needed, 6 deferred (primitive hardening — same class 1.10 deferred for TimePicker/AspectRatio), ~7 dismissed (incl. two Blind Hunter token false-alarms: `--shadows-N` and bare `--spacing` are both defined tokens). |
| 2026-06-13 | code-review fixes (Oleksii chose apply patch + both fixes): (1) Combobox keyboard handler skips navigation keys on an empty filtered list (no `NaN`/`-1` highlight); (2) Pagination guards `limit < 1` → `null` and clamps `page` to `[1, totalPageCount]`; (3) `BreadcrumbLink` gained an `asChild`/Slot escape hatch via `@radix-ui/react-slot@1.2.5` (review-sanctioned dep, added exact-pinned to `packages/ui` + lockfile). 3 new tests (pagination ×2, combobox ×1, breadcrumb ×1 → 77 ui tests). Full gate sweep re-run green after the fixes. The 6 deferred items remain deferred (logged in deferred-work.md). |

## Review Findings

_From bmad-code-review on 2026-06-13 (branch `TOOLS-1-11/design-system-molecule-parity`, diff vs `main`). All quality gates independently re-verified green; no blocking findings. Items below are robustness/parity hardening on design-system primitives._

- [x] [Review][Decision→Fixed] Pagination now validates caller-supplied `page`/`limit` — `limit < 1` returns `null` (no `Infinity`/`NaN` page count); `page` is clamped to `[1, totalPageCount]` so an out-of-range/stale page renders the nearest valid page and `onPageChange` emits an in-range neighbour. Decided (Oleksii): the primitive defensively guards. Added tests `renders nothing when the limit is not positive` + `clamps an out-of-range page into the valid range`. [packages/ui/src/components/molecules/pagination/Pagination.tsx]
- [x] [Review][Decision→Fixed] `BreadcrumbLink` gained an `asChild`/Slot escape hatch — `asChild` renders the consumer's element via `@radix-ui/react-slot` `Slot` (reference parity; lets 1.5 auth / future nav inject `NavigationLink` without `next/link` coupling in `ui`). Decided (Oleksii): add now; `@radix-ui/react-slot@1.2.5` recorded as a review-sanctioned dep (AC-3 table). Added test `renders the consumer element when asChild is set`. [packages/ui/src/components/molecules/breadcrumb/Breadcrumb.tsx]
- [x] [Review][Patch→Fixed] Combobox keyboard handler no longer produces `NaN`/`-1` highlight on an empty filtered list — navigation keys (everything but `Escape`) are skipped when `optionCount === 0`. Added test `ignores arrow navigation when the filtered list is empty`. [packages/ui/src/components/molecules/combobox/hooks/use-combobox-keyboard.ts]
- [x] [Review][Defer] Combobox carries `role="combobox"` on BOTH the trigger button and the inner search input — ARIA smell (two combobox roles when open), but mirrors the reference and the open-state axe pass is green. [packages/ui/src/components/molecules/combobox/Combobox.tsx] — deferred, parity-consistent a11y hardening
- [x] [Review][Defer] `BreadcrumbEllipsis` has no `sr-only` "More" label — collapsed crumbs are invisible to AT (the icon is `aria-hidden`); the shadcn reference includes a visually-hidden label. [packages/ui/src/components/molecules/breadcrumb/Breadcrumb.tsx] — deferred, minor a11y enhancement
- [x] [Review][Defer] `Card` `withAction` grid layout does not engage when `<CardAction>` is wrapped in a Fragment — `Children.toArray(...).some(child => child.type === CardAction)` does not descend into a Fragment. Direct-child usage works. [packages/ui/src/components/molecules/card/Card.tsx:17-18] — deferred, edge composition, reference-consistent
- [x] [Review][Defer] Combobox controlled `value` not present in `optionList` renders the placeholder (and `aria-label` falls back to placeholder) instead of signalling a set-but-unresolved value — e.g. async-loaded options or stale selection. [packages/ui/src/components/molecules/combobox/Combobox.tsx] — deferred, reference parity
- [x] [Review][Defer] `FieldError` multi-error dedup keys by `error.message`, so multiple message-less (`undefined`) entries collapse to one Map key and render a single-item `<ul>` where the single-message plain-text fast-path was intended. [packages/ui/src/components/molecules/field/Field.tsx:104-116] — deferred, markup quirk on a rare RHF shape
- [x] [Review][Defer] Combobox trigger does not open-and-highlight on `ArrowDown`/`Enter`/`Space` (keyboard handler is wired only to the search input inside the popover); only Radix `Trigger` click/Enter toggles open. [packages/ui/src/components/molecules/combobox/Combobox.tsx] — deferred, keyboard nicety, reference parity
