...
Adversarial code review (bmad-code-review — Blind Hunter + Edge Case Hunter + Acceptance Auditor) of `main..HEAD`. **Verdict: APPROVE. Zero MUST-FIX findings.** Orchestrator re-ran gates: `type-check`, `lint`, `i18n:parity` all green. All 10 ACs met; all merge-blocking hard rules honored (no hand-written fetch, generated-client-only, controller→service→repository, en/uk parity, tests shipped, no ranged deps, no eslint/prettier, no `example/` imports, DI untouched). Migration+backfill, defaults idempotency/duplicate-`Other` disambiguation, the `USER_RESPONSE_COLUMNS`+`updateScoped` silent-failure trap, and the `resolveOnboardedProfile` gate across all 9 pages verified correct.

Nice-to-have items (non-blocking):

- [ ] [Review][Patch] Consolidate duplicated `CURRENCY_OPTION_LIST` — byte-identical `src/app/[locale]/transactions/constants/currency-option-list.ts` (imported by `TransactionForm`) still coexists with the new app-wide `src/constants/currency-option-list.ts` (imported by `CurrencyStep`+`ProfileForm`); re-point `TransactionForm` to the app-wide copy and delete the transactions copy (contradicts `shared-constants-no-duplication`; pre-existing, but the lift was the moment to consolidate). [apps/money-tracker/src/app/[locale]/transactions/constants/currency-option-list.ts]
- [ ] [Review][Patch] `CurrencyStep` field-error uses `translateError(errors.defaultCurrency.message)` without the `.has()` fallback used by every other error render (action-error Alert, `CategoriesStep`); deterministic today because `z.enum(..., 'currencyRequired')` guarantees the key, but brittle if the schema message config changes. [apps/money-tracker/src/app/[locale]/onboarding/components/currency-step/CurrencyStep.tsx]
- [ ] [Review][Patch] No automated guard that advancing the currency step preserves the `?step=categories` query — `router.replace('/onboarding?step=categories')` (bare string) works live (next-intl preserves query; confirmed by QA capture) and in the mocked unit test, but nothing exercises the real localized router's query handling. [apps/money-tracker/src/app/[locale]/onboarding/components/currency-step/CurrencyStep.tsx]
- [ ] [Review][Patch] AC10 evidence/record mismatch — the 15 committed PNGs are an asymmetric sample (several states are single-theme or single-viewport), while the Dev Agent Record states "across light/dark × mobile(390)/desktop," implying a fuller matrix than committed. Either complete the light+dark×mobile+desktop matrix per interactive state or soften the record wording to "sampled." Operator's call for a PoC. [_bmad-output/implementation-artifacts/visual-qa/5-3-onboarding-flow/]
- [x] [Review][Defer] Deep-link `?step=categories` without a saved currency renders the currency step but leaves the URL reading `step=categories` (downgrade-in-place, not a redirect). Functionally correct (no bypass; StepIndicator + form both show currency; submit re-navigates cleanly), matches the documented "guard folded into `resolveStep`" approach — cosmetic URL/spec-wording nit only. [apps/money-tracker/src/app/[locale]/onboarding/page.tsx]
- [x] [Review][Defer] By-design (documented out-of-scope): a non-onboarded user who signs *in* (vs up) lands on the ungated `/` placeholder and is only routed into onboarding on the first click into a gated page. [apps/money-tracker/src/app/[locale]/sign-up/SignUpFormSection.tsx]

Dismissed as non-issues: zod-version dependence of `z.enum` message shorthand (zod is `4.4.3` where the 2nd string arg IS the message; repo-wide precedent); server actions not self-hardening SDK rejections (by-design — every caller wraps with `.catch`, per the story's stated rejection-hardening pattern).
